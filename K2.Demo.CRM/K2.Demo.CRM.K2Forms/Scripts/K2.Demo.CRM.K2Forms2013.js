...
function showSimpleK2ProcessInformation(data) {
    ...
    if (K2_CONFIG_SECTION_PROCESSINFORMATION) {
        folioRow = k2valueRowTemplateNoIcon.replace(/\[\[k2value\]\]/g, data.ProcessInstance.Folio).replace(/\[\[k2valuelabel\]\]/g, "Folio").replace(/\[\[k2valueid\]\]/g, "k2folio");
        processInstanceRow = k2valueRowTemplateNoIcon.replace(/\[\[k2value\]\]/g, data.ProcessInstance.FullName).replace(/\[\[k2valuelabel\]\]/g, "Process Name").replace(/\[\[k2valueid\]\]/g, "k2processname");
        processInstanceDateRow = k2valueRowTemplateNoIcon.replace(/\[\[k2value\]\]/g, formatJSONDate(data.ProcessInstance.StartDate)).replace(/\[\[k2valuelabel\]\]/g, "Process Start Date").replace(/\[\[k2valueid\]\]/g, "k2processstartdate");
    }

    if (K2_CONFIG_SECTION_ACTIVITYINFORMATION) {
        activityRow = k2valueRowTemplateNoIcon.replace(/\[\[k2value\]\]/g, data.ActivityInstanceDestination.Name).replace(/\[\[k2valuelabel\]\]/g, "Activity Name").replace(/\[\[k2valueid\]\]/g, "k2activity");
        activityDateRow = k2valueRowTemplateNoIcon.replace(/\[\[k2value\]\]/g, formatJSONDate(data.ActivityInstanceDestination.StartDate)).replace(/\[\[k2valuelabel\]\]/g, "Activity Start Date").replace(/\[\[k2valueid\]\]/g, "k2activitystartdate");
    }

    if (K2_CONFIG_SECTION_VIEWFLOW) {
        viewflowRow = k2valueRowTemplateHyperlink.replace(/\[\[k2value\]\]/g, K2VIEWFLOWURL + data.ProcessInstance.ID).replace(/\[\[k2valuemask\]\]/g, "K2 View Flow").replace(/\[\[k2valuelabel\]\]/g, "View Flow").replace(/\[\[k2valueid\]\]/g, "k2viewflow");
    }
...
function showFullK2ProcessInforation(data) {
    ...
    if (K2_CONFIG_VIEWFLOW) {
        tabViewFlow = tabViewFlowTemplate.replace(/\[\[viewflowurl\]\]/g, K2VIEWFLOWURL + data.ProcessInstance.ID);
    }
    ...
    if (K2_CONFIG_ACTIONS || K2_CONFIG_ACTIVITYINFORMATION || K2_CONFIG_PROCESSINFORMATION) {
        ...
        if (K2_CONFIG_PROCESSINFORMATION) {
            tabDetailsProcess = tabProcessInfoTemplate.replace(/\[\[processname\]\]/g, data.ProcessInstance.Name).replace(/\[\[processnamelabel\]\]/g, "Process Name").replace(/\[\[processnameid\]\]/g, "processinstancename").replace(/\[\[processstartdate\]\]/g, formatJSONDate(data.ProcessInstance.StartDate)).replace(/\[\[processstartdatelabel\]\]/g, "Process Start Date").replace(/\[\[processstartdateid\]\]/g, "processinstancestartdate");
            tabDetailsProcess += tabFolioTemplate.replace(/\[\[folio\]\]/g, data.ProcessInstance.Folio).replace(/\[\[foliolabel\]\]/g, "Folio").replace(/\[\[folioid\]\]/g, "processinstancefolio");
        }

        if (K2_CONFIG_ACTIVITYINFORMATION) {
            tabDetailsActivity = tabActivityInfoTemplate.replace(/\[\[activityname\]\]/g, data.ActivityInstanceDestination.Name).replace(/\[\[activitynamelabel\]\]/g, "Activity").replace(/\[\[activitynameid\]\]/g, "activityinstancename").replace(/\[\[activitystartdate\]\]/g, formatJSONDate(data.ActivityInstanceDestination.StartDate)).replace(/\[\[activitystartdatelabel\]\]/g, "Activity Start Date").replace(/\[\[activitydatestartid\]\]/g, "activityinstancestartdate");
        }
...
function actionK2WorklistItem2013() {
    return $.ajax({
        url: K2WFSERVICESBASEURI + 'REST.svc/Worklist/Items(' + K2SN + ')/Actions(' + K2SELECTEDACTION + ')/Execute?$format=json',
        method: 'GET',
        contentType: "application/json; charset=utf-8",
        dataType: "JSON",
        async: true,
