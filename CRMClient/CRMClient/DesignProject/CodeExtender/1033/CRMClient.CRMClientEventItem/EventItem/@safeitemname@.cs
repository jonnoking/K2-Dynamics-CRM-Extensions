        ...
        public void Main($contexttype$ K2)
        {
            ...
            try
            {
                using (SourceCode.Workflow.Common.HostedServers.SmartObjects smartObjects = new SmartObjects(K2.Configuration.SmartObjectServer))
                {
                    ...
                    smartObject.Properties["Owner"].Value = K2.Configuration.TaskOwner;
                    ...
                    string taskxml = K2.ProcessInstance.XmlFields["CRM Tasks"].Value;
                    if (string.IsNullOrEmpty(taskxml))
                    {
                        taskxml = "<CRMTasks></CRMTasks>";
                    }
                    XmlDocument tasks = new XmlDocument();
                    tasks.LoadXml(taskxml);
